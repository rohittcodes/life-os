...
export async function upsertWellness(fd: FormData) {
  ...
  const moodRaw = parseInt(fd.get("mood") as string)
  const mood = moodRaw > 0 ? moodRaw : null
  const energyRaw = parseInt(fd.get("energy") as string)
  const energy = energyRaw > 0 ? energyRaw : null
