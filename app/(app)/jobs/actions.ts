...
export async function updateJob(id: string, formData: FormData) {
  ...
  if (!company || !role || !VALID_STATUSES.has(status)) return

  const salary_raw = parseInt(formData.get("salary_lpa") as string)
  const applied_at = (formData.get("applied_at") as string) || null
  const next_action_date = (formData.get("next_action_date") as string) || null
  ...
  await supabase
    ...
    .update({
      ...
      role,
      status,
      salary_lpa: !isNaN(salary_raw) && salary_raw > 0 ? salary_raw : null,
